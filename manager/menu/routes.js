      ...
      return new Promise((resolve, reject) => {
			...
			pool.query(query, function(err, result) {
				...
				for (let i = 0; i < result.rowCount; ++i) {
					...
					item_ingredients.push(JSON.stringify({product_id: result.rows[i].product_id, quantity: result.rows[i].quantity}));
				}
				menu_ingredients.push(item_ingredients);
				return resolve();
			});
...
router.put("/update_ingredients", function(req, res) {
	...
	var item_ingredients = request.item_ingredients.filter((ingredient) => (JSON.parse(ingredient).product_id != ""));
    ...
    query2 = () => {
		return new Promise((resolve, reject) => {
			...
			var values = []
			var count = 1;
			for (let i = 0; i < request.item_ingredients.length; ++i) {
				var item_ing = JSON.parse(request.item_ingredients[i]);
				if (item_ing.product_id == "") continue;
				query += '(\'' + menu[request.idx].menu_item_id + '\',$' + count + ', $' + (count + 1) + '),';
				count += 2;
				values.push(item_ing.product_id);
...
router.put("/update_menu", function(req, res) {
    ...
    if (row.idx == menu.length && row.menu_item_id != "") {  // new item, make sure menu_item_id is the value that was changed
		...
		var item_ingredients = [];
		item_ingredients.push(JSON.stringify({product_id: '1', quantity: '0'}));
		menu_ingredients.push(item_ingredients);
        var query = "INSERT INTO menu VALUES ($1, 'None', 0)";
        pool.query(query, [row.menu_item_id], function(err, result) {
            if (err) throw err;
        });
		// another query to update menu_item ingredients
